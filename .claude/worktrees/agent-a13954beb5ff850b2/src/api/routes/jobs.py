...
async def get_job_step_outputs(request, job_id: uuid.UUID):
    """Return all persisted step outputs for a job, ordered by step_id."""
    job = await Job.objects.filter(id=job_id).afirst()
    if not job:
        raise HttpError(404, "Job not found")

    outputs = [
        so async for so in StepOutput.objects.filter(job_id=job_id).order_by("step_id")
    ]
