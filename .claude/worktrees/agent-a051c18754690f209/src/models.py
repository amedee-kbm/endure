...
class StepOutput(models.Model):
    """Records the result of an individual step() call within a pipeline stage.

    step_id is a global counter across the entire job execution (not reset per
    stage), so (job_id, step_id) uniquely identifies one step call.  On resume
    after a crash the worker initialises its counter from MAX(step_id)+1 and
    step() skips any position that already has a row here.
    """
    job = models.ForeignKey(
        Job, on_delete=models.CASCADE, related_name="step_outputs"
    )
    step_id = models.IntegerField()
    step_name = models.TextField()
    output = models.TextField()                    # JSON-serialised return value
